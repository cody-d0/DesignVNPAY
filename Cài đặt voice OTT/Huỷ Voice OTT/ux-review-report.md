...
| **Hiện tại** | Bottom-sheet OTP (7103) chỉ có 6 ô digit-input + button "Xác nhận". Không có đồng hồ đếm ngược, không có "Gửi lại OTP" sau timeout. |
| **Đề xuất** | Thêm countdown timer (DDL `countdown-timer-1`: `timeLeft` state, `isExpired`) bên dưới ô OTP. Thêm "Gửi lại OTP" button, disable cho đến khi hết 60 giây. Spec: `resendTimeout=60s` theo DDL `otp-input-1`. |
| **DDL Ref** | COMP:otp-input-1 — `canResend`, `timeLeft`, `resendTimeout=60`; COMP:countdown-timer-1 |
| **Heuristic** | Nielsen #1 Visibility of System Status — user không biết OTP còn hiệu lực bao lâu |
| **Effort** | Medium |
...
| **Hiện tại** | "Quý khách vui lòng đăng ký nhận thông báo BĐSD **trước** đăng ký nhận thông báo BĐSD bằng giọng nói" — phrase "đăng ký nhận thông báo BĐSD" lặp 2 lần, không có CTA rõ ràng hướng user đến đăng ký BĐSD. |
| **Đề xuất** | Rewrite: "Quý khách cần đăng ký nhận thông báo BĐSD trước khi sử dụng tính năng Voice OTT. Vui lòng đăng ký tại mục Thông báo và Cảnh báo." Thêm CTA "Đến đăng ký thông báo" thay cho "Đồng ý" chung. |
| **DDL Ref** | UXG-243 — CTA label phải phản ánh đúng hành động; Banking product: Trust paramount |
| **Heuristic** | Nielsen #6 Recognition rather than Recall — nội dung phải đủ rõ không cần user đoán |
| **Effort** | Low |
...
| **Hiện tại** | 7105 được navigate từ "Chỉnh sửa" (SCR-VOT-001) nhưng CTA vẫn là "Đăng ký" — gây nhầm lẫn: user đã đăng ký rồi nên "Đăng ký" tạo cảm giác sẽ tạo service mới. |
| **Đề xuất** | Thay "Đăng ký" bằng "Lưu thay đổi" khi màn hình được mở từ context chỉnh sửa. Hoặc: "Xác nhận" nếu đơn giản hơn. |
| **DDL Ref** | UXG-243 — CTA clarity; COMP:app-header-1 — title context |
| **Heuristic** | Nielsen #4 Consistency and Standards — CTA label phải consistent với context |
| **Effort** | Low |
...
| **Hiện tại** | Fields "Từ" và "Đến" hiển thị rỗng, không có placeholder "HH:mm", không có validation error state khi Từ ≥ Đến hoặc chỉ điền 1 field. |
| **Đề xuất** | Thêm placeholder "HH:mm" cho cả 2 fields. Thêm inline error: "Giờ bắt đầu phải nhỏ hơn giờ kết thúc" khi Từ ≥ Đến. DDL `text-input-1` spec: `error` state available. |
| **DDL Ref** | COMP:text-input-1 — `error` state, `helper text`; UXG-165 |
...
| **Hiện tại** | Popup confirm (7102): cả 2 action đều dùng màu blue text — "Huỷ" (secondary) và "Đồng ý" (primary). Với destructive action, action chính xác nhận huỷ nên được phân biệt rõ (thường màu đỏ hoặc warning). |
| **Đề xuất** | Đổi màu button "Đồng ý" trong confirm dialog thành destructive color (DDL `base.destructive`). Hoặc đổi label thành "Huỷ dịch vụ" để rõ hơn ý định. |
| **DDL Ref** | TOKEN:base.destructive; Banking: "Trust paramount" — destructive actions cần visual warning |
| **Heuristic** | Nielsen #5 Error Prevention — phân biệt rõ destructive vs neutral actions |
| **Effort** | Low |
...
| **Hiện tại** | Link "Nghe thử" chỉ có icon play_circle tĩnh. Khi tap, không có indication audio đang phát (playing state) hay đã dừng. |
| **Đề xuất** | Thêm playing state: icon đổi sang pause, text đổi sang "Đang phát..." với animated indicator. Stop state khi hoàn thành. |
| **DDL Ref** | UXG-165 — System status visibility for media |
...
| **Hiện tại** | Block giới thiệu dài (Voice OTT description + warning + Nghe thử) xuất hiện identically trên cả SCR-VOT-001 và SCR-VOT-002. User đã đọc ở màn 1, thấy lại y hệt ở màn 2 — tốn screen space. |
| **Đề xuất** | SCR-VOT-002 rút ngắn intro box: chỉ giữ warning "Lưu ý: KHÔNG để thiết bị im lặng" và "Nghe thử". Remove duplicated description text. |
| **DDL Ref** | UXG-165 — Content density optimization |
...
| **Hiện tại** | Khi chưa có tài khoản nào được thêm, section "Tài khoản đọc loa thông báo" chỉ hiển thị "+ Thêm tài khoản" — không có empty state text hay hint về nên thêm bao nhiêu tài khoản. |
| **Đề xuất** | Thêm placeholder text: "Chưa có tài khoản nào. Nhấn + để thêm tài khoản nhận đọc loa." Validation: require ≥1 tài khoản trước khi enable CTA. |
| **DDL Ref** | COMP:empty-state-1; UXG-165 — Empty state clarity |
...
| 15 | OTP close affordance (bottom-sheet) | Skill C (vision) | COMP:otp-input-1; UXG-165 | ⚠️ Unverifiable | Từ ảnh 7103: bottom-sheet có title "Xác thực giao dịch" — close X từ vision metadata có nhưng không rõ kích thước/vị trí trong ảnh static |
...
| 17 | Fitts's Law — OTP cell touch target | Skill A (law: fitts) | COMP:otp-input-1 cellSize=48 | ⚠️ Unverifiable | DDL spec: cellSize=48px. Từ ảnh 7103: 6 cells trong ~343px width → est. ~57px per cell. Borderline OK nhưng không đo được chính xác từ screenshot |
...
| 20 | Loading state cho OTP submit | Skill B (banking: security) | COMP:app-header-1 | ⚠️ Unverifiable | Static artboard không thể xác nhận button loading state khi submit OTP |
...
| 9 | Calendar icon touch target (Fitts's Law) | Skill A (law: fitts) | COMP:text-input-1; UXG-165 | ⚠️ Unverifiable | DDL yêu cầu ≥44px. Từ ảnh 7105 calendar icons trông nhỏ (~24px icon trong 72px field) — field có vẻ đủ target nhưng icon hit area không verify được |

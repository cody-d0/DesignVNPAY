...
- **DDL (call on demand):** [.cursor/docs/ddl/contract.md](.cursor/docs/ddl/contract.md), [.cursor/docs/ddl/hierarchy.md](.cursor/docs/ddl/hierarchy.md). Conventions § DDL reference: [.cursor/skills/figma-to-prd-md/conventions.md](.cursor/skills/figma-to-prd-md/conventions.md).
...
- **Bước 1 — Spec "before":** Từ prd_augmented (hoặc PRD trước khi áp dụng prd_extension): Section 3 Mô tả màn hình + (tùy chọn) block "Đề xuất cải thiện UX (chưa áp dụng)" từ prd_extension/ocr_ux_improvements. Ghi file spec .md (Mockup Screen Spec). Tham chiếu: `.cursor/skills/md-to-mockup/SKILL.md` § Mockup Screen Spec.

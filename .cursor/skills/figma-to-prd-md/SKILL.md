...
**ui-ux-pro-max:** Tham chiếu [.cursor/docs/ui-ux-pro-max-reference.md](.cursor/docs/ui-ux-pro-max-reference.md) (canonical path data: `.agents/skills/ui-ux-pro-max/data/`).
...
L4 (UI Implementation — token resolution, CSS specs) do `prd-full-pipeline` Skill 3-4 xử lý. Ref: [.cursor/docs/ddl/contract.md](.cursor/docs/ddl/contract.md), [.cursor/docs/ddl/views-and-outputs.md](.cursor/docs/ddl/views-and-outputs.md).

Skill này **gọi** design data layer (DDL) on-demand theo scope GLOBAL + PRODUCT để lấy UX rules, tokens, component mapping; không sở hữu DDL. Hierarchy: [.cursor/docs/ddl/hierarchy.md](.cursor/docs/ddl/hierarchy.md). Tham chiếu DDL khi sinh ocr_ux_improvements (Phase 2g): query scope GLOBAL (ux-guidelines.csv, web-interface.csv, ux-laws.csv tại `.agents/skills/ui-ux-pro-max/data/`), gắn mỗi đề xuất với rule tương ứng (`ddl_ref`) và citation trong output; chi tiết tại Phase 2g và 2g-bis.
...
**Chạy tiếp Phase 2 → 5 (mặc định):** Sau khi hoàn thành Phase 0 (nếu chạy) và Phase 1, agent **luôn mặc định chạy tiếp Phase 2 → 3 → 4 → 5** đến khi pipeline kết thúc (Phase Report đã ghi). Không dừng giữa chừng để hỏi user "có chạy tiếp không"; chỉ dừng khi gate fail (vd. OCR_GATE_FAILED) hoặc user **rõ ràng** yêu cầu dừng.

...
Tài liệu này là **vị trí tham chiếu đầy đủ duy nhất** cho skill ui-ux-pro-max. Chỉ có **một** đường dẫn canonical; mọi tham chiếu trong repo dùng path bên dưới (không dùng `.cursor/skills/ui-ux-pro-max/`). Các skill khác (figma-to-prd-md, design-system-gen, pdr-analyze, comp-extraction, v.v.) link tới file này hoặc dùng path canonical trực tiếp.

...
2. **Match registry**: Load `.agents/skills/ux-signal-inference/data/text-signals-generic.json` + `text-signals-{domain}.json`. Match pattern (instruction, counter, action, cancel, tags) → inferred_ux: `flows[]`, `components[]`, `states[]`.
...
## Pre-Skill-C Gate: VISION_READY

**BAT BUOC verify truoc khi chay Skill C:**

| # | Check | Severity |
|:--|:---|:---:|
| V1 | `wireframe_images[]` populated trong `prd_ddl_report` per screen | 🔴 BLOCK |
| V2 | Tat ca image paths resolve to existing files (`fs.existsSync`) | 🔴 BLOCK |
| V3 | It nhat 1 anh tim thay (`total_images > 0`) | 🔴 BLOCK |
| V4 | Test doc duoc anh (view_file thu 1 PNG) | 🔴 BLOCK |

**Neu fail:** DUNG — khong chay Skill C. Thong bao: "Khong tim thay anh wireframe cho vision review. Kiem tra lai thu muc ui/ hoac chay lai export."

**Ly do:** Neu khong co anh thuc, Skill C se "hallucinate" evidence tu PRD text/Figma metadata thay vi doc anh thuc. (Case study: Co-op Bank KHCN — ui/ trong nhung report van co vision evidence.)

Khi chay tu workflow `figma-to-ux-review`: gate nay la mot phan cua pipeline gates. Khi chay standalone: skill tu enforce.

---

## Skill C: Vision Review + Suy luan + De xuat cai tien

**Chay SAU khi A va B hoan thanh VA VISION_READY PASS.** Doc anh, doi chieu voi 2 report, suy luan them, cross-ref DDL, cho de xuat cai tien cu the.

### Input

- `signal_ddl_report[]` tu Skill A
- `prd_ddl_report[]` tu Skill B
- `ui/*.png` (wireframes) — buoc duy nhat doc anh — **DA VERIFY TON TAI qua VISION_READY**
...
**Khi chay tu workflow `figma-to-ux-review`:** Du degraded hay khong, findings phai qua HC-5 (Vision Review Confirmation) truoc khi ghi report cuoi cung. User co the reject false positives hoac yeu cau bo sung coverage.

---

## Data & Reference (reuse, khong tao moi)

- **ux-signal-inference**: `.agents/skills/ux-signal-inference/SKILL.md` — goi nguyen ven (Skill A)
- **Pattern registry**: `.agents/skills/ux-signal-inference/data/text-signals-generic.json`, `text-signals-{domain}.json`
- **DDL GLOBAL**: `.agents/skills/ui-ux-pro-max/data/` — ux-guidelines.csv, web-interface.csv, ux-laws.csv
- **review-ux checklist**: `.agents/skills/review-ux/checklist.md` — 9 nhom lam framework phan loai
- **DDL contract**: `.agents/docs/ddl/contract.md`, `.agents/docs/ddl/hierarchy.md`

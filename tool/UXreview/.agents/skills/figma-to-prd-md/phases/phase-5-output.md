...
### Pre-output IMAGE_EXIST check (mới — sync từ workflow HC-4)

**Thực hiện sau OCR check, trước khi ghi `.handoff/`.** Verify file PNG tồn tại trên filesystem:

1. Với mọi screen có `wireframe_images` không rỗng: kiểm tra file .png tương ứng tồn tại tại path `{section}/ui/{filename}.png` hoặc `{output_dir}/{section}/ui/{filename}.png`.
2. Verify file size > 0 cho mọi PNG.
3. Nếu ảnh thiếu:
   - **Khi chạy từ workflow `figma-to-ux-review`:** `IMAGE_MISSING_FOR_SCREEN_X` → **BLOCK** handoff (HC-4 fail). Vẫn ghi .md pack nhưng không ghi `.handoff/`.
   - **Khi chạy standalone:** `IMAGE_MISSING_WARNING` → ghi warning trong overview, vẫn ghi cả .md lẫn `.handoff/`.

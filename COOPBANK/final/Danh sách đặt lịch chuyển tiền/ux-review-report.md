...
| **Guideline** | Accessibility — Color Only (DDL #180) |
| **UX Law** | — |

**Hiện trạng (Current State)**

Trạng thái giao dịch (Hoạt động, Đã hủy, Hết hạn, Tạm dừng) chỉ được phân biệt bằng màu chữ mà không có icon hoặc badge kèm theo. User với color vision deficiency sẽ không phân biệt được trạng thái.

**Hệ quả (Consequences)**

- User dalto không phân biệt được Hoạt động (xanh) vs Tạm dừng (cam) vs Đã hủy (đỏ)
- Vi phạm WCAG 2.1 AA — Use of Color (1.4.1)
- Risk: User thao tác nhầm trên giao dịch đã hủy/hết hạn

**Đề xuất giải pháp (Proposed Solution)**

Thêm icon prefix trước trạng thái text:
- ✅ Hoạt động (xanh + check icon)
- ⏸ Tạm dừng (cam + pause icon)
- ❌ Đã hủy (đỏ + cancel icon)
- ⏰ Hết hạn (xám + clock icon)
...
| **Guideline** | Interaction — Confirmation Dialogs (DDL #35) |
| **UX Law** | — |

**Hiện trạng (Current State)**

Popup xác nhận "Tạm dừng" chỉ có text "Thông báo" + message, với 2 nút "Không" hoặc "Đồng ý". Popup không hiển thị thông tin giao dịch cụ thể (mã, số tiền, tên) nào sẽ bị tạm dừng. User dễ thao tác nhầm khi có nhiều giao dịch.

**Hệ quả (Consequences)**

- User xác nhận nhầm → tạm dừng/hủy giao dịch sai
- Giao dịch tài chính → mất tiền hoặc trễ lịch chuyển
- Banking domain yêu cầu confirm dialog phải có đủ context

**Đề xuất giải pháp (Proposed Solution)**

Popup xác nhận cần hiển thị: Tên thụ hưởng + Số tiền + Mã đặt lịch để user verify trước khi đồng ý. Thêm highlight text "Hành động này không thể hoàn tác" cho Hủy.
...
|-------|-------|
| **Màn hình** | Danh sách đặt lịch chuyển tiền › Chi tiết |
| **Guideline** | Interaction — Error Feedback (DDL #33) |
| **UX Law** | — |

**Hiện trạng (Current State)**

OTP bottom sheet hiển thị chung chung "Quý khách vui lòng nhập mã PIN của Soft OTP để xác thực giao dịch" nhưng không hiện rõ consequence (tạm dừng hay hủy). User nhập OTP mà không biết đang xác nhận hành động gì.

**Hệ quả (Consequences)**

- Confusion: user không biết OTP này xác nhận Tạm dừng hay Hủy
- Nếu nhập sai 5 lần → khóa Soft OTP, tổn hại trải nghiệm

**Đề xuất giải pháp (Proposed Solution)**

OTP title nên ghi rõ "Xác thực Tạm dừng giao dịch" hoặc "Xác thực Hủy giao dịch" tùy action. Hiển thị mã đặt lịch bên dưới title.
...
| **Guideline** | Layout — Content Jumping (DDL #19) |
| **UX Law** | — |

**Hiện trạng (Current State)**

Hai empty states ("Không có GD đang Hoạt động" và "Không có kết quả tìm kiếm") hiển thị text centered nhưng không có illustration hoặc icon. Nội dung text empty state 1 quá dài ("Qúy khách không có giao dịch đặt lịch chuyển tiền nào đang Hoạt động. Vui lòng sử dụng bộ lọc để truy vấn các giao dịch đặt lịch chuyển tiền có trạng thái khác.") → khó đọc, không có CTA trực tiếp.

**Hệ quả (Consequences)**

- User không biết phải làm gì tiếp (no clear CTA)
- Text dài centered khó đọc
- Empty state thiếu illustration → cảm giác lỗi/trống

**Đề xuất giải pháp (Proposed Solution)**

Thêm illustration icon (empty box hoặc calendar icon), rút ngắn text, thêm CTA button "Mở bộ lọc" để user tap trực tiếp thay vì phải tự tìm icon filter.
...
Bộ lọc "Theo thời gian" có 2 date picker (Từ ngày, Đến ngày) nhưng không hiển thị validation khi Từ ngày > Đến ngày. Annotation text giải thích logic truy vấn nhưng font size nhỏ, khó đọc. Không có default value cho date fields.

**Hệ quả (Consequences)**

- User có thể chọn khoảng thời gian invalid → API error
- Annotation text nhỏ → user bỏ qua → query sai expectation
- Empty date fields → confusion

**Đề xuất giải pháp (Proposed Solution)**

Pre-fill date: Từ ngày = 30 ngày trước, Đến ngày = hôm nay. Validation inline khi Từ > Đến. Annotation text cỡ chữ lớn hơn hoặc dùng info icon (i) tooltip.
...
|-------|-------|
| **Màn hình** | Danh sách đặt lịch chuyển tiền › Danh sách |
| **Guideline** | Touch — Touch Target Size (DDL #22) |
| **UX Law** | Fitts's Law |

**Hiện trạng (Current State)**

Icon filter ở header có kích thước nhỏ (24×24, ~tap area có thể < 44×44). Đây là action quan trọng trên màn danh sách.

**Hệ quả (Consequences)**

- Khó tap trên mobile, đặc biệt user lớn tuổi (target banking)
- Vi phạm guideline touch target minimum 44×44px

**Đề xuất giải pháp (Proposed Solution)**

Tăng touch target padding cho filter icon lên min 44×44px (giữ visual size 24×24).
...
|-------|-------|
| **Màn hình** | Danh sách đặt lịch chuyển tiền › Chi tiết |
| **Guideline** | Interaction — Success Feedback (DDL #34) |
| **UX Law** | — |

**Hiện trạng (Current State)**

Khi hoàn tất OTP → redirect về manage screen với popup "Thành công". Nhưng popup thành công trên manage screen không hiện thông tin giao dịch nào đã thay đổi (mã, tên).

**Hệ quả (Consequences)**

- User không confirm được đúng giao dịch nào đã bị tạm dừng
- Nếu có nhiều GD, không biết GD nào đã thay đổi

**Đề xuất giải pháp (Proposed Solution)**

Popup thành công hiển thị thêm: Mã đặt lịch + Tên thụ hưởng + Trạng thái mới.
...
Danh sách giao dịch đã thực hiện (bottom sheet "list.png") hiển thị lịch sử GD nhưng các entry "Không thành công", "Tạm dừng", "Hủy" không có mã giao dịch và phí giao dịch — chỉ có ngày + trạng thái. Trong khi "Thành công" có đầy đủ thông tin (mã, phí).

**Hệ quả (Consequences)**

- Inconsistency trong data display giữa các trạng thái
- User muốn tra cứu GD thất bại không có mã để liên hệ CSKH

**Đề xuất giải pháp (Proposed Solution)**

Hiển thị mã giao dịch cho tất cả entries (kể cả thất bại). Nếu không có mã → hiện "—" thay vì ẩn hoàn toàn.
...
| 5 | Loading states | DDL guideline #10 | Animation > Loading | ⚠️ Unverifiable | Không thể verify loading state từ static design |
...
| 10 | Dropdown feedback | DDL guideline #29 | Interaction > Hover | ⚠️ Unverifiable | Không verify dropdown expanded state |

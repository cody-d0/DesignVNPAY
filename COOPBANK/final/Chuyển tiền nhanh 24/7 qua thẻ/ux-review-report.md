...
| **Mức độ** | 🔴 Critical |
| **DDL** | UXG-235 · COMP:otp-input-1 |
| **UX Law** | doherty |

**🔍 Hiện trạng**

Màn OTP overlay (CTNST_5) hiển thị 6 ô nhập OTP nhưng thiếu hoàn toàn: (1) nút "Gửi lại mã OTP" với countdown timer, (2) thông báo thời gian hết hạn OTP, (3) trạng thái loading khi verify. DDL spec `otp-input-1` yêu cầu state `canResend`, `timeLeft`, `digits` + child "resend button with countdown timer". Từ ảnh: chỉ thấy 6 ô trống + nút "Xác nhận", không có bất kỳ UI resend nào.

> Evidence: DDL `COMP:otp-input-1` spec: states `canResend`, `timeLeft`. Screenshot CTNST_5: 6 static OTP cells, NO resend button, NO countdown timer.

**⚠️ Hậu quả**

- **User impact:** Nếu OTP không nhận được hoặc hết hạn, user bị kẹt — không có cách gửi lại mã → phải thoát và bắt đầu lại toàn bộ flow chuyển tiền
- **Business impact:** Drop-off cao tại bước cuối cùng → mất conversion chuyển tiền, tăng support call
- **Violation:** DDL `COMP:otp-input-1` — thiếu mandatory states. UX Law `doherty` — response feedback absent

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Resend Button | Thêm text link "Gửi lại mã OTP" với countdown "Gửi lại sau 60s" | Dưới 6 ô OTP, trên nút Xác nhận |
| 2 | Countdown Timer | Hiển thị "Mã OTP hết hạn sau: 02:00" realtime | Dưới instruction text |
| 3 | Loading State | Spinner + "Đang xác thực..." khi tap Xác nhận | Replace nút Xác nhận |
...
Không có trạng thái error/validation visible trên form nhập. Từ ảnh CTNST_1 và CTNST_3: tất cả input fields chỉ có 2 state — empty và filled. Không thấy state error (viền đỏ, icon cảnh báo, helper text lỗi). DDL `text-input-1` spec yêu cầu states: `error`, `errorMessage`, `helperText`. Đặc biệt với nghiệp vụ banking: số thẻ 16 chữ số cần validate format, số tiền cần validate min/max/limit.

> Evidence: DDL `COMP:text-input-1` spec: states `error`, `errorMessage`. Screenshots CTNST_1, CTNST_3: no error state variant, no red border, no error icon.

**⚠️ Hậu quả**

- **User impact:** User nhập sai số thẻ/số tiền → không biết lỗi ở đâu → frustration, phải đoán
- **Business impact:** Tăng tỷ lệ giao dịch thất bại, tăng support call
- **Violation:** DDL `COMP:text-input-1` — thiếu error states. UXG-180 — form validation feedback

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Error State | Viền đỏ (#dc2626) + icon cảnh báo + helper text lỗi dưới mỗi field | Tất cả input fields |
| 2 | Inline Validation | Validate real-time: số thẻ 16 digits, số tiền > 0 và ≤ hạn mức | Mỗi field khi blur |
| 3 | Error Summary | Banner tổng hợp lỗi nếu tap "Tiếp tục" khi có lỗi | Trên CTA button |
...
Hai variant xác nhận (CTNST_4: qua thẻ, CTNST_5: qua số điện thoại) hiển thị thông tin khác nhau về label: CTNST_4 dùng "Số thẻ thụ hưởng" còn CTNST_5 dùng "Số tài khoản thụ hưởng" + thêm "Số điện thoại". Thông tin không nhất quán giữa 2 variant — user có thể nhầm lẫn đang xác nhận giao dịch nào.

> Evidence: Screenshot CTNST_4: "Số thẻ thụ hưởng: 9704000012345678". Screenshot CTNST_5: "Số tài khoản thụ hưởng: 13237899903" + "Số điện thoại: 0906198882". Label và data khác nhau giữa 2 variants.

**⚠️ Hậu quả**

- **User impact:** Confusion — user không chắc chắn đang xác nhận chuyển tiền qua thẻ hay qua tài khoản
- **Business impact:** Sai lầm giao dịch → dispute, refund cost
- **Violation:** UXG-183 — consistency trong confirmation screens

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Header subtitle | Thêm subtitle phân biệt: "Chuyển tiền qua thẻ" vs "Chuyển tiền qua số điện thoại" | Dưới title "Xác nhận giao dịch" |
| 2 | Label consistency | Giữ nguyên label "Số thẻ thụ hưởng" cho flow qua thẻ, không mix với "Số tài khoản" | Info rows |
...
Biên nhận chỉ hiển thị "Mã giao dịch: 0982312" (7 ký tự) — format không chuẩn, thiếu prefix bank. Không có QR code hoặc barcode để tra cứu. DDL `receipt-preview-1` spec có child `qr_code` optional. Thời gian giao dịch "22:00 15/02/2020" — format cũ, không rõ timezone.

> Evidence: DDL `COMP:receipt-preview-1`. Screenshot CTNST_6: mã GD "0982312", no QR code. Timestamp "22:00 15/02/2020" without timezone.

**⚠️ Hậu quả**

- **User impact:** Khó tra cứu giao dịch sau này, mã ngắn dễ trùng
- **Business impact:** Tăng thời gian support khi tra soát giao dịch
- **Violation:** Peak-End Rule — kết thúc flow nên tạo ấn tượng tích cực, mã GD chuẩn tăng confidence

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Transaction Code | Format chuẩn: "COOP-2020021522-0982312" (prefix + date + code) | Receipt row "Mã giao dịch" |
| 2 | Timestamp | Format: "22:00 — 15/02/2020 (GMT+7)" | Receipt row "Thời gian" |
...
Overlay "Danh bạ thụ hưởng" (CTNST_2) hiển thị danh sách dài nhưng không có trạng thái empty state khi search không có kết quả. Ngoài ra, không thấy indicator/tip rằng có thể "kéo xuống" để xem thêm. Từ ảnh: danh sách hiển thị 8 contacts, bị cắt ở cuối (Chị Linh Vũ bị cắt).

> Evidence: Screenshot CTNST_2: Danh bạ thụ hưởng overlay, 8 contacts visible. Last item "Chị Linh Vũ" partially truncated. No empty-state variant shown.

**⚠️ Hậu quả**

- **User impact:** Nếu search không ra → user không biết có phải nhập tay hay không
- **Business impact:** User bỏ cuộc chuyển tiền
- **Violation:** UXG-184 — empty state guidance. Hick's Law — long list without clear navigation

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Empty State | "Không tìm thấy người hưởng. Bạn có thể nhập trực tiếp số thẻ." + CTA "Nhập thủ công" | Center of overlay khi search empty |
| 2 | Scroll Indicator | Thêm shadow/fade ở bottom khi list dài hơn viewport | Bottom của danh sách |
...
Nội dung giao dịch mặc định hiển thị "NGUYEN HOANG KHAI chuyen tien" (CTNST_1) — toàn chữ hoa không dấu, style banking truyền thống. Nội dung "Merry Christmas" (CTNST_3) thì lại có chữ thường. Không nhất quán style giữa 2 variants.

> Evidence: Screenshot CTNST_1: "NGUYEN HOANG KHAI chuyen tien" (all caps no diacritics). Screenshot CTNST_3: "Merry Christmas" (mixed case). Inconsistent text style.

**⚠️ Hậu quả**

- **User impact:** Aesthetic inconsistency, giảm cảm giác premium
- **Business impact:** Nhẹ — chỉ ảnh hưởng visual polish
- **Violation:** UXG-181 — text formatting consistency

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Default Content | Auto-generate nội dung mặc định: "{Tên} chuyển tiền" (có dấu, sentence case) | Input "Nội dung giao dịch" |
...
Biên nhận hiển thị "nguyen tung" (chữ thường) trong khi CTNST_4 confirm screen hiển thị "NGUYEN TUNG" (chữ hoa). Ngân hàng "BIDV - Ngân hàng đầu tư và phát triển Việt Nam" bị xuống 3 dòng trên receipt card (từ ảnh CTNST_6 rõ ràng bị wrap thành 3 lines).

> Evidence: Screenshot CTNST_6: "nguyen tung" (lowercase) vs CTNST_4 "NGUYEN TUNG" (uppercase). Bank name wraps to 3 lines on receipt card.

**⚠️ Hậu quả**

- **User impact:** Thiếu tự tin về tính chính xác khi tên hiển thị khác nhau giữa 2 màn
- **Business impact:** Nhẹ — aesthetic issue
- **Violation:** Peak-End Rule — biên nhận nên tạo ấn tượng tốt, text wrap giảm readability

**✅ Giải pháp đề xuất**

| # | Component | Đề xuất | Vị trí |
|---|:---|:---|:---|
| 1 | Name Casing | Normalize tên thụ hưởng: luôn uppercase cả receipt lẫn confirm | Receipt row "Tên người thụ hưởng" |
| 2 | Bank Name | Viết tắt trên receipt: "BIDV" (đã rõ bên confirm). Hoặc tăng width card | Receipt row "Ngân hàng" |
...
| 17 | Label "Số tài khoản/Số thẻ/Số điện thoại" placeholder quá dài | Skill C | UXG-181 | ⚠️ Unverifiable | Chỉ thấy trong CTNST_2 (overlay), có thể bị truncate trên device nhỏ hơn 375px |
...
| 11 | Timestamp format rõ ràng | Skill C | — | ⚠️ Unverifiable | "22:00 15/02/2020" — không rõ timezone, nhưng có thể server-side logic |
